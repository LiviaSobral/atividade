...
public class TaskDAO {
    ...
    public static ArrayList<Task> listTasksOfUser(int idUser){
        ...
        String sql = "SELECT taskID,title,description,date,status FROM tasks WHERE userID = ? ORDER BY date ASC";
        try(Connection con = ConnectionSQL.connect()){
            PreparedStatement prep = con.prepareStatement(sql);
    ...
    public static ArrayList<Task> listTasksOfUser(int idUser,String status){
        ArrayList<Task> tasks = new ArrayList<>();
        String sql = "SELECT taskID,title,description,date,status FROM tasks WHERE userID = ? AND status = ? ORDER BY date ASC";
        try(Connection con = ConnectionSQL.connect()){
            PreparedStatement prep = con.prepareStatement(sql);
            prep.setInt(1, idUser);
